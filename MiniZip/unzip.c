...
/*
  Locate the Central directory of a zipfile (at the end, just before
    the global comment)
*/
local uint64_t unz64local_SearchCentralDir OF((const zlib_filefunc64_32_def* pzlib_filefunc_def, voidpf filestream));
...
/*
  Locate the Central directory 64 of a zipfile (at the end, just before
    the global comment)
*/
local uint64_t unz64local_SearchCentralDir64 OF((
    const zlib_filefunc64_32_def* pzlib_filefunc_def,
    voidpf filestream));
        ...
        while(acc < file_info.size_file_extra)
        {
            ...
            if (headerId == 0x0001)
            {
                                                        uLong uL;
                                                                ...
                                                                if(file_info.uncompressed_size == (uint64_t)(unsigned long)-1)
                                                                {
                                                                        if (unz64local_getLong64(&s->z_filefunc, s->filestream,&file_info.uncompressed_size) != UNZ_OK)
                                                                                        err=UNZ_ERRNO;
                                                                }

                                                                if(file_info.compressed_size == (uint64_t)(unsigned long)-1)
                                                                {
                                                                        if (unz64local_getLong64(&s->z_filefunc, s->filestream,&file_info.compressed_size) != UNZ_OK)
                                                                                  err=UNZ_ERRNO;
                                                                }

                                                                if(file_info_internal.offset_curfile == (uint64_t)(unsigned long)-1)
                                                                {
